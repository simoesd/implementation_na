...
pub fn inRange(point: &Vec<f64>, ranges: &Vec<Vec<(f64, f64)>>) -> bool {
    for range in ranges {
        let inAllDimensions = point.iter().zip(range.iter()).fold(true, |inAllDimensions, (p_a, (r_1, r_2))| inAllDimensions && (p_a >= r_1 &&  p_a <= r_2));
        if inAllDimensions {
            return true;
        }
    }
...
pub fn abovePlane(point: &Vec<f64>, normalVector: &Vec<f64>) -> bool {
    ...
    let matrixProduct = hyperplaneMatrix.dot(&pointMatrix) - hyperplaneMatrix.dot(&(hyperplaneMatrix.map(|x| x*bias)));
