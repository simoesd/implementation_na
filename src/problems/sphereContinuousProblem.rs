...
impl NAProblem for SphereContinuousNAProblem {
    fn evaluate(&self, nn: &NANN) -> (bool, f64, Array2<f64>) {
        ...
        let inputs: Vec<f64> = self.ranges.iter().fold(vec![], |mut acc, (x_1, x_2)| {acc.extend([x_1, x_2]); acc});
        let prediction: Array2<f64> = (*nn).clone().forward(Array2::<f64>::from_shape_vec(Ix2(self.ranges.len(), 2usize), inputs).unwrap());
        let givenRanges = Interval::fromVec(self.ranges.clone());
        let predictionRanges = Interval::fromVec(prediction.rows().into_iter().map(|row| {
            let angle: f64 = *row.index(0);
            let bias: f64 = mathUtils::ring(*row.index(1), 1.0, true);
